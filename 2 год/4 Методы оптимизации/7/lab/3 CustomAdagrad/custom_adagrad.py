    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for param in group["params"]:
                ...
                if len(state) == 0:
                    state["sum"] = torch.zeros_like(param)

                state["sum"].addcmul_(grad, grad, value=1.0)

                denominator = state["sum"].sqrt().add_(eps)
                param.addcdiv_(grad, denominator, value=-lr)
...
def main():
    ...
    plot_history(history, "Обучение модели с CustomAdagrad")
