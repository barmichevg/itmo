...
def bisection(f, a, b, eps, max_iter=1000, verbose=True):
    fa, fb = f(a), f(b)
    if fa * fb > 0:
        return None, 0, "На концах интервала нет смены знака"

    if verbose:
        print("\nЭтап: метод половинного деления")
    ...
    while abs(b - a) > eps and it < max_iter:
        ...
        if verbose:
            print(f"Итерация {it}: x = {x:.10f}, f(x) = {fx:.10f}, |b-a| = {abs(b-a):.10f}")
...
def newton(f, df, x0, eps, max_iter=1000, verbose=True):
    x = x0
    ...
    for it in range(1, max_iter + 1):
        ...
        if verbose:
            print(
                f"Итерация {it}: x = {x:.10f}, f(x) = {fx:.10f}, "
                f"f'(x) = {dfx:.10f}, x_next = {x_next:.10f}"
            )
...
def simple_iter_eq(f, df, a, b, eps, max_iter=1000, verbose=True):
    ...
    if verbose:
        print("\nЭтап: метод простой итерации")
        print(f"Интервал: [{a:.6f}; {b:.6f}]")
        print(f"lambda = {lambd:.10f}")
        print(f"q = {q:.10f}")
    ...
    if verbose:
        print(f"Начальное приближение: x0 = {x:.10f}")
    ...
    for it in range(1, max_iter + 1):
        ...
        if verbose:
            print(
                f"Итерация {it}: x = {x:.10f}, "
                f"x_next = {x_next:.10f}, |dx| = {abs(x_next - x):.10f}"
            )
...
def simple_iter_sys(system, x0, y0, eps, max_iter=1000, verbose=True):
    q = convergence_q(system, x0, y0)

    if verbose:
        print("\nЭтап: решение системы методом простой итерации")
        print(f"Начальное приближение: x0 = {x0:.10f}, y0 = {y0:.10f}")
        print(f"q = {q:.10f}")
    ...
    for it in range(1, max_iter + 1):
        ...
        if verbose:
            print(
                f"Итерация {it}: x = {x:.10f}, y = {y:.10f}, "
                f"x_next = {x_next:.10f}, y_next = {y_next:.10f}, "
                f"|dx| = {dx:.10f}, |dy| = {dy:.10f}"
            )
...
def solve_equation(eq_num, method, a, b, eps):
    ...
    print(f"Этап: проверка числа корней на интервале -> найдено: {roots}")
    ...
    if method == 1:
        ...
        root, it, err = bisection(eq["f"], a, b, eps, verbose=True)
    elif method == 3:
        method_name = "Ньютона"
        x0 = (a + b) / 2
        print(f"Начальное приближение для метода Ньютона: x0 = {x0:.10f}")
        root, it, err = newton(eq["f"], eq["df"], x0, eps, verbose=True)
    else:
        method_name = "простой итерации"
        root, it, err = simple_iter_eq(eq["f"], eq["df"], a, b, eps, verbose=True)
...
def solve_system(sys_num, x0, y0, eps):
    ...
    x, y, it, errs, err = simple_iter_sys(system, x0, y0, eps, verbose=True)
